"use strict";
// TODO: Add support for serializing Arrays (break it down and reconstruct?)
...
var Serializer = /** @class */ (function () {
    ...
    // TODO: add type declarations for this.
    Serializer.prototype.decodeString = function (data) {
        ...
        if (data[0] <= 255) {
            len = data[0];
            return {
                value: new TextDecoder().decode(data.slice(1, 1 + len)),
                bytesRead: 1 + len,
            };
        }
        len = (data[0] << 8) | data[1];
        return {
            value: new TextDecoder().decode(data.slice(2, 2 + len)),
            bytesRead: 2 + len,
        };
    };
    ...
    Serializer.prototype.encode = function (data) {
        ...
        if (typeof data === "number") {
            ...
            for (var i = 0; i < data.length; i++) {
                //? deconstruct the array, and rebuild it
                if (typeof data === "number") {
                    results.push(this.encodeInteger(data));
                }
                else if (typeof data === "string") {
                    results.push(this.encodeString(data));
                }
                /*
                switch(typeof data) {
                    case "number": {
                    ...
                    default:
                        console.log("unsupported type passed to encode through array");
                        break;
                }
                        */
            }
        ...
        return Array.isArray(res)
            ? res
            : combineUint8Arrays(new Uint8Array([type]), res);
    };
